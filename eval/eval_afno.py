...
class EvalConfig:
    ...
    ckpt_path: str = "best_afno_tiny.pth"
    weight_source: str = "auto"  # "auto" | "eval_model" | "ema" | "model"

    output_dir: str = "eval_results"
    model_tag: str = "afno_tiny"
...
_AFNO_BUILDERS = {
    "afno_tiny":  A.afno_tiny,
    "afno_small": A.afno_small,
    "afno_base":  A.afno_base,
}

def build_model_from_ckpt_config(
    ckpt_cfg: Dict[str, Any], num_classes: int, img_size: int,
) -> nn.Module:
    model_variant = ckpt_cfg.get("model_variant") or "afno_tiny"
    if model_variant not in _AFNO_BUILDERS:
        raise ValueError(
            f"Unknown AFNO variant: {model_variant}. "
            f"Supported: {list(_AFNO_BUILDERS)}."
        )
    factory = _AFNO_BUILDERS[model_variant]

    model = factory(
        num_classes=num_classes,
        img_size=ckpt_cfg.get("img_size", img_size),
        sparsity_thresh=ckpt_cfg.get("sparsity_thresh", 0.01),
        hard_thresh_frac=ckpt_cfg.get("hard_thresh_frac", 1.0),
        drop_path_rate=0.0,
...
def main(cfg: EvalConfig):
    ...
    model_variant = ckpt_cfg.get("model_variant") or "afno_tiny"
    pc = A.count_params(model)
    print(
        f"[Model] {model_variant} "
        f"total_params={pc['total']:,} ({pc['total']/1e6:.2f}M)  "
        f"filter_only={pc['filter_only']:,}  "
        f"sparsity_thresh={ckpt_cfg.get('sparsity_thresh','?')}  "
        f"hard_thresh_frac={ckpt_cfg.get('hard_thresh_frac','?')}  "
    ...
    summary = {
        ...
        "model_info": {
            ...
            "sparsity_thresh": ckpt_cfg.get("sparsity_thresh"),
            "hard_thresh_frac": ckpt_cfg.get("hard_thresh_frac"),
