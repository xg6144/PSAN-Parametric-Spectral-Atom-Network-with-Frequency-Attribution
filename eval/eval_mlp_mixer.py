...
train_mlp_mixer.py 로 학습한 checkpoint (best_*.pth / last_*.pth)를 로드해
test split에서 종합 메트릭 / 부트스트랩 CI / confusion matrix / per-class /
FLOPs / latency 를 한 번에 계산한다.
...
MLP-Mixer 특화 사항
  - variant: mlp_mixer_tiny | mlp_mixer_small | mlp_mixer_base
  - 평가 시 drop_path_rate / use_checkpoint 은 0 / False 로 강제.
"""
import sys
import os
import json
...
from models import mlp_mixer as M
...
class EvalConfig:
    ...
    # checkpoint
    ckpt_path: str = "best_mlp_mixer_tiny.pth"
...
# Model rebuild
# ------------------------------------------------------------
# 체크포인트의 config로 MLP-Mixer를 동일 variant로 재구성.
# 평가 시 drop_path_rate / use_checkpoint 은 0 / False 로 강제.
# ============================================================
_MIXER_BUILDERS = {
    "mlp_mixer_tiny":  M.mlp_mixer_tiny,
    "mlp_mixer_small": M.mlp_mixer_small,
    "mlp_mixer_base":  M.mlp_mixer_base,
}
...
def build_model_from_ckpt_config(
    ...
    model_variant = ckpt_cfg.get("model_variant") or "mlp_mixer_tiny"
    if model_variant not in _MIXER_BUILDERS:
        raise ValueError(
            f"Unknown MLP-Mixer variant: {model_variant}. "
            f"Supported: {list(_MIXER_BUILDERS)}."
        )
    factory = _MIXER_BUILDERS[model_variant]
...
def main(cfg: EvalConfig):
    ...
    summary = {
        ...
        "model_info": {
            ...
            "patch_size": int(model.patch_embed.patch_size),
            "grid": int(grid),
            ...
            "token_mix_params": pc["token_mix"],
            "channel_mix_params": pc["channel_mix"],
...
if __name__ == "__main__":
    cfg = EvalConfig(
        ...
        ckpt_path="best_mlp_mixer_tiny.pth",
        weight_source="auto",
