...
train_gfnet.py 로 학습한 checkpoint (best_*.pth / last_*.pth)를 로드해
...
class EvalConfig:
    ...
    # checkpoint
    ckpt_path: str = "best_gfnet_tiny.pth"
...
# Model rebuild
# ------------------------------------------------------------
# 체크포인트에 저장된 config로 GFNet을 동일하게 재구성.
# 평가 시에는 drop_rate / drop_path_rate를 0으로 강제.
# (train_gfnet.py는 현재 gfnet_tiny만 지원)
# ============================================================
_GFNET_BUILDERS = {
    "gfnet_tiny": gfnet_tiny,
}
...
def build_model_from_ckpt_config(
    ...
    # train_gfnet.py의 TrainConfig에는 backbone_name 필드가 없고 model_tag만 있다.
    # backbone은 gfnet_tiny로 고정하되, 향후 variant가 추가되면 backbone_name을
    # checkpoint config에 저장해 스위칭할 수 있도록 구조만 맞춰둔다.
    backbone_name = ckpt_cfg.get("backbone_name") or "gfnet_tiny"
    if backbone_name not in _GFNET_BUILDERS:
        raise ValueError(
            f"Unknown GFNet backbone: {backbone_name}. "
            f"Supported: {list(_GFNET_BUILDERS)}."
        )
    model_fn = _GFNET_BUILDERS[backbone_name]
    model = model_fn(
        num_classes=num_classes,
        img_size=ckpt_cfg.get("img_size", img_size),
        drop_rate=0.0,         # 평가 시 stochastic 요소 제거
        drop_path_rate=0.0,
    )
    return model
...
def measure_flops(model, device, img_size):
    """fvcore 기반 FLOPs 측정. MACs = FLOPs / 2."""
    try:
