...
class EvalConfig:
    ...
    # checkpoint
    ckpt_path: str = "best_deit_tiny.pth"
...
# Model rebuild
# ------------------------------------------------------------
# 체크포인트에 저장된 config로 timm DeiT 모델을 동일하게 재구성.
# 평가 시에는 drop_rate / drop_path_rate를 0으로 강제.
# ============================================================
def build_model_from_ckpt_config(ckpt_cfg: Dict[str, Any], num_classes: int) -> nn.Module:
    backbone_name = ckpt_cfg.get("backbone_name", "deit_tiny_patch16_224")
    model = timm.create_model(
        backbone_name,
        pretrained=False,       # 체크포인트 가중치를 직접 로드하므로 pretrained 불필요
...
def main(cfg: EvalConfig):
    ...
    print(f"[Model] {ckpt_cfg.get('backbone_name','deit_tiny_patch16_224')} "
    ...
    summary = {
        ...
        "model_info": {
            "backbone": ckpt_cfg.get("backbone_name", "deit_tiny_patch16_224"),
